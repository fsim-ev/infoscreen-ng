			...
			loop {
				...
				for calopt in calopts.iter() {
					...
					events.extend(cal.events.into_iter()
						.filter(|sec| sec.get_end()
							.and_then(dt_convert)
							.is_some_and(|ts| ts > (now.date_naive() + Duration::days(1))
								.and_time(Default::default()).timestamp() as _)
						)
					);
				...
				ui.upgrade_in_event_loop(move |ui| {
					fn set_list<T: 'static>(mrc: ModelRc<T>, mut list: Vec<T>) {
...
impl TryInto<ui::Event> for &icalendar::Event {
	...
	fn try_into(self) -> Result<ui::Event> {
		let ev = ui::Event {
			summary: self.get_summary().context("summary missing")?.into(),
			location: self.get_location().unwrap_or_default().into(),
			categories: self.property_value("CATEGORIES")
				.map(|s| s.replace(',', ", ").to_owned().into())
				.unwrap_or_default(),
			time_start: self.get_start()
				.ok_or_else(|| anyhow::format_err!("missing start time"))
				.and_then(dt_convert)?,
			time_end: self.get_end()
				.ok_or_else(|| anyhow::format_err!("missing start time"))
				.and_then(dt_convert)
				.unwrap_or_default(),
...
impl TryInto<ui::Task> for &icalendar::Todo {
	type Error = anyhow::Error;

	fn try_into(self) -> Result<ui::Task> {
		let task = ui::Task {
			summary: self.get_summary().context("summary missing")?.into(),
			progress: self.get_percent_complete()
				.map(|percent| percent as _)
				.unwrap_or_default(),
			time_start: self.get_start()
				.ok_or_else(|| anyhow::format_err!("missing start time"))
				.and_then(dt_convert)
				.unwrap_or_default(),
			time_due: self.get_end()
				.ok_or_else(|| anyhow::format_err!("missing start time"))
				.and_then(dt_convert)
				.unwrap_or_default(),

			complete: self.get_completed().is_some(),
			organiser: Default::default(),
...
impl TryInto<Task> for &icalendar::Todo {
	type Error = anyhow::Error;

	fn try_into(self) -> Result<Task> {
		let task = Task {
			summary: self.get_summary().context("summary missing")?.into(),
			progress: self.get_percent_complete()
				.map(|percent| percent as _)
				.unwrap_or_default(),
			time_start: self.get_start().map(|dpt| ),
			time_due: self.get_end().and_then(dtp_convert),

			complete: self.get_completed(),
			organiser: Default::default(),
			color: Default::default(),
		};
		Ok(task)
...
fn dtp_convert(cdt: DatePerhapsTime) -> Result<DateTime<Local>> {
	match cdt {
		DatePerhapsTime::DateTime(dt) => 
			dt.try_into_utc()
				.map(|dt| dt.with_timezone(&Local))
